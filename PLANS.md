...
- `P3.1b` — **CERRADO (6 de agosto de 2026).** 16 tests de interacción real (`@testing-library/react` + `user-event` + `jsdom`) contra `Home()`, API mockeada con match exacto y falla ruidosa ante ruta no registrada (`tests/support/fetch-mock.mjs`). Cubre crear/abrir/controlar un proyecto, retry/rework/escalate, aprobaciones, conectividad, y los campos de sólo lectura que P4.3/P4.4 van a necesitar (last_error, veredicto, evidencia de test report, decisiones, artifacts, métricas). Tres problemas reales del arnés (no de `page.tsx`) encontrados y corregidos corriendo el mecanismo, no diseminados de antemano: `navigator` getter-only en Node, el singleton `screen` de `@testing-library/dom` resuélto una sola vez al importarse, resolución de `node_modules` rota desde `os.tmpdir()`. Sin refactor de `page.tsx`, sin cambios visuales/funcionales, sin response_model=. ADR 0031.
...
**P3.1b — CERRADO (6 de agosto de 2026).** Entregables 1 y 3 (tests de interacción de UI contra API mock, estados/campos que P4 va a consumir). 16 tests nuevos bajo `tests/`, `node --test "tests/**/*.test.mjs"` los descubre solo (una línea en `package.json`). Detalle completo, incluidos los tres problemas reales del arnés de test encontrados corriendo el mecanismo, en `docs/decisions/0031-*`.
