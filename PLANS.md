...
agregue sin tocar el harness otra vez. Una ronda de revisión directa
corrigió la primera propuesta antes de implementar: eliminó una
extracción de backend que la propia investigación no justificaba
directamente, exigió el diseño acíclico en vez de un ciclo runtime, y
exigió el mecanismo Vite general en vez de un transpiler hardcodeado a
dos archivos -- "si investigar demuestra que un refactor no hace falta,
no se hace sólo porque estaba escrito como un paso del roadmap".
...
`.\test.ps1` completo en verde (396 backend + 18 web). Detalle completo,
incluidos los tres problemas reales de Vite encontrados corriendo el
mecanismo (puerto WebSocket, servidor sin cerrar, costo de pre-bundling
en frío), en ADR 0033.
