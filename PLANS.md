...
- Próximo paso: **P4.4 — entrega y auditoría** (P4.1, P4.2 y P4.3 completos).
...
> Lee `CLAUDE.md`/las instrucciones del repo y `PLANS.md` completos. Verifica que estás sobre `main` actualizado y limpio. No reabras P0, P1, P2, P3.1 (a+b), P3.2, P3.3, P3.4, P4.1, P4.2 ni P4.3 (a+b) salvo una regresión demostrable o un requisito concreto de P4.4+ que lo justifique (ver el disparador de revisión en ADR 0033 para el caso de backend, la condición futura en ADR 0034 para habilitar ejecución aislada, el límite de carpetas planas sin snapshot transaccional en ADR 0035, la falta de tracking incremental "desde la última exportación" en ADR 0036, y las dos limitaciones explícitas de ADR 0037 -- recover sólo ofrece artefactos propios del item, sin badge de conteo en vivo en el nav). Empieza por **P4.4** — entrega y auditoría -- ver la sección P4.4 del roadmap para el alcance ya definido; P4.1 (importar), P4.2 (exportar) y P4.3 (reparar) ya están cerrados, así que hay un historial real de intentos/artefactos/decisiones sobre el que construir el informe final.
